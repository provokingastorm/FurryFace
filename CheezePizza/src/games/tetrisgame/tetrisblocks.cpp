...
TetrisBlockS::TetrisBlockS(float TopLeftPositionX, float TopLeftPositionY)
{
    ...
    {
        m_Blocks[i]->SetColor(PURPLE);
    }
    
    m_BlockType = BlockType_S;
}
...
int TetrisBlockS::GetBottomLine() const
{
    int BottomLine = 0;
...
void TetrisBlockS::SimulateRotation(int Rotation, TetrisBlockProxy& OutProxy)
{
    switch( Rotation % 4 )
    {
    case 1:
		{
			...
			OutProxy.Proxies[3].Line = OutProxy.Proxies[2].Line;

			const int BaseLinePosition = OutProxy.Proxies[0].LinePosition;
	        
			OutProxy.Proxies[1].LinePosition = BaseLinePosition + 1;
    ...
    case 3:
		{
			...
			OutProxy.Proxies[1].LinePosition = BaseLinePosition - 1;
			OutProxy.Proxies[2].LinePosition = BaseLinePosition;
			OutProxy.Proxies[3].LinePosition = BaseLinePosition + 1;
    ...
    case 0:
		{
			...
			OutProxy.Proxies[3].LinePosition = OutProxy.Proxies[0].LinePosition - 1;
